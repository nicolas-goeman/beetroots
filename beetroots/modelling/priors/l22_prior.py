...
class L22SpatialPrior(SpatialPrior):
    ...
    def neglog_pdf(
        ...
        if self.list_edges.size > 0:
            neglog_p += xp.sum(
                (Var[self.list_edges[:, 1], :] - Var[self.list_edges[:, 0], :])
                ** 2,
                axis=0,
            )
        if with_weights:
    ...
    def neglog_pdf_one_pix(
        ...
        Returns
        -------
        nl_priors : xp.ndarray of shape (N_candidates,)
        ...
        neglog_p = xp.zeros_like(list_pixel_candidates)  # (N_candidates, D)

        list_edges_pix = self.list_edges[(self.list_edges[:, 0] == n)]

        if list_edges_pix.size > 0:
            neglog_p += (
                Var[list_edges_pix[:, 1], :] - Var[list_edges_pix[:, 0], :]
            ) ** 2  # (N_candidates, D)
        neglog_p = neglog_p * self.weights[None, :]
        neglog_p = xp.sum(neglog_p, axis=1)
        return neglog_p  # (D,)
    ...
    def hess_diag_neglog_pdf(self, Var: xp.ndarray) -> xp.ndarray:
        ...
        if self.list_edges.size > 0:
            ...
            hess_diag[idx, :] += 2 * counts[:, None] * xp.ones((idx.size, self.D))
